...
pub fn variables_rendering_test() {
  ...
  gc.expression_to_string(complex_var)
  |> should.equal("some_long_variable_name")
}
...
pub fn pipe_operators_test() {
  ...
  gc.expression_to_string(double_piped)
  |> should.equal("42 |> some_function |> another_function")
}
...
pub fn complex_expressions_test() {
  ...
  gc.expression_to_string(logic_expr)
  |> should.equal("x > 0 && y < 100 || z == 42")
}
...
pub fn string_escaping_test() {
  ...
  gc.expression_to_string(quote_string)
  |> should.equal("\"He said \\\"Hello!\\\"\"")
  gc.expression_to_string(newline_string) |> should.equal("\"Line 1\\nLine 2\"")
  gc.expression_to_string(backslash_string)
  |> should.equal("\"C:\\\\path\\\\to\\\\file\"")
}
