...
fn workflow_demo() {
  ...
  let string_module = parser.GleamModule(
    name: "gleam/string",
    functions: [
  ...
  io.println("✅ Parsed " <> string.inspect(list.length(string_module.functions)) <> " functions from gleam/string")
  ...
  let processed_input = gc.variable("trimmed_input") // Would be GenString.trim(input_var)
  let greeting = op.append(greeting_prefix, processed_input)
  
  // This simulates: GenString.length(greeting)
  let greeting_length = gc.variable("greeting_length") // Would be GenString.length(greeting)
  
  io.println("📝 Generated expressions:")
  io.println("   - Input processing: " <> gc.expression_to_string(processed_input))
  io.println("   - Greeting creation: " <> gc.expression_to_string(greeting))
  io.println("   - Length calculation: " <> gc.expression_to_string(greeting_length))
  ...
  let generated_file = gc.file(["generated", "user_helpers"], [
    process_user_input,
    calculate_length
  ])
