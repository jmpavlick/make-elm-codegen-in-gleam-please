...
fn generate_simple_value_binding(func: parser.GleamFunction) -> c.Declaration {
  let value_expr = c.simple_expression(
    c.Variable(func.name),
    string_to_gleam_type(func.return_type)
  )
  
  c.Declaration(c.DeclarationDetails(
    name: func.name,
    exposed: c.Exposed,
    imports: [],
...
fn generate_function_call_binding(func: parser.GleamFunction, args: List(#(String, String))) -> c.Declaration {
    ...
    to_body: fn(_index) {
      c.DeclarationBody(
        declaration: c.FunctionDecl(
          name: func.name,
          args: list.map(args, fn(arg) { #(arg.0, Some(c.CustomType([], "Expression", []))) }),
...
fn generate_function_call_body(func_name: String, args: List(#(String, String))) -> c.GleamExpression {
...
fn generate_call_record(functions: List(parser.GleamFunction), module_name: String) -> c.Declaration {
  let call_fields = list.map(functions, fn(func) {
    let field_name = func.name
    let field_value = case func.args {
      [] -> c.Variable(func.name) // No args, just the value
      args -> {
        // Create a function that takes expressions
        let lambda_args = list.map(args, fn(arg) { c.VariablePattern(arg.0) })
        let call_expr = c.FunctionCall(
          c.Variable(func.name),
...
fn generate_values_record(functions: List(parser.GleamFunction), module_name: String) -> c.Declaration {
  let value_fields = list.map(functions, fn(func) {
    let field_name = func.name
    let field_value = c.StringLiteral(func.name) // String representation of the function name
    #(field_name, field_value)
  })
    ...
    "Result(" <> rest -> {
      ...
      case string.split_once(rest, ",") {
        Ok(#(ok_type, err_part)) -> {
          ...
          c.ResultType(string_to_gleam_type(ok_type), string_to_gleam_type(err_type))
        }
        Error(_) -> c.CustomType([], type_str, [])
      }
    }
    other -> c.CustomType([], other, []) // Default to custom type
  }
}

/// Generate bindings for an entire package
pub fn generate_package_bindings(package_path: String) -> Result(List(c.Declaration), String) {
...
pub fn generate_gen_module_file(module: parser.GleamModule) -> String {
  ...
  "// Generated bindings for " <> module.name <> "\n// TODO: Implement full rendering"
}